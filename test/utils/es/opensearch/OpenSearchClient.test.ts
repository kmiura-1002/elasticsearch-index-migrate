...
import { openSearchClientContainer } from '../../ioc-test';
import chaiAsPromised from 'chai-as-promised';
import { Index } from 'es6/api/requestParams';

chai.use(chaiAsPromised);

describe('OpenSearchClient test', () => {
    const client = openSearchClientContainer().get<ElasticsearchClient>(
        Bindings.ElasticsearchClient
    );

    it('version check', () => {
        expect(client.version()).is.eq('opensearch');
    });
